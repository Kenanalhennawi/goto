...
export function FilesSearchClient({
  ...
  return (
    <>
      <section className="mb-5 content-card border-t-2 border-t-navy p-5">
        ...
        <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row lg:w-auto">
            <input
              ...
              className="min-w-0 rounded-md border border-border bg-white px-3.5 py-2 text-sm text-ink placeholder:text-ink-faint focus:border-accent"
            />
            {selectedCategory === "Files" && (
              <select
                value={selectedType}
                onChange={(event) => setSelectedType(event.target.value)}
                className="rounded-md border border-border bg-white px-3 py-2 text-sm font-medium text-ink focus:border-accent"
              >
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {categoryOrder.map((category) => (
            <button
              ...
              className={`rounded border px-2.5 py-1.5 text-[13px] font-semibold transition-colors ${
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  <span className="rounded-sm border border-blue-200 bg-sky-soft px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky">
                    {link.reference_category}
                  </span>
                  <span className="rounded-sm border border-border bg-slate-50 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-ink-muted">
                    {link.file_type}
                  </span>
              ...
              <div className="flex shrink-0 gap-2">
                <button
                  ...
                  disabled={!link.url}
                  className="inline-flex items-center justify-center rounded border border-border bg-white px-3.5 py-1.5 text-[13px] font-semibold text-ink-muted transition-colors hover:border-accent hover:text-accent disabled:cursor-not-allowed disabled:opacity-45"
                >
                ...
                {link.url ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center justify-center rounded border border-blue-200 bg-sky-soft px-3.5 py-1.5 text-[13px] font-semibold text-sky transition-colors hover:border-sky hover:bg-white"
                  >
                    Open
                  </a>
                ) : (
                  <span className="inline-flex items-center justify-center rounded border border-amber-200 bg-amber-50 px-3.5 py-1.5 text-[13px] font-semibold text-amber-700">
                    No link
                  </span>
                )}
              </div>
